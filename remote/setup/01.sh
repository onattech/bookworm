...
# Prompt to enter a password for the PostgreSQL bookworm user (rather than hard-coding
# a password in this script).
read -p "Enter password for bookworm DB user: " DB_PASSWORD
...
# Set up the bookworm DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE bookworm"
sudo -i -u postgres psql -d bookworm -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d bookworm -c "CREATE ROLE bookworm WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the bookworm database to the system-wide environment
# variables in the /etc/environment file.
echo "BOOKWORM_DB_DSN='postgres://bookworm:${DB_PASSWORD}@localhost/bookworm'" >> /etc/environment
